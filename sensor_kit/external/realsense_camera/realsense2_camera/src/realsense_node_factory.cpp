...
#define REALSENSE_ROS_EMBEDDED_VERSION_STR                                            \
    (VAR_ARG_STRING(VERSION : REALSENSE_ROS_MAJOR_VERSION.REALSENSE_ROS_MINOR_VERSION \
...
RealSenseNodeFactory::RealSenseNodeFactory(const rclcpp::NodeOptions &node_options)
    : RosNodeBase("camera", "/camera", node_options), _logger(this->get_logger())
{
...
RealSenseNodeFactory::on_configure(const rclcpp_lifecycle::State &state)
{
    RCLCPP_INFO(get_logger(),
                "🚀 Lifecycle Transition | FROM: %s (%d) → TO: CONFIGURING",
                state.label().c_str(),
                state.id());
    init();
...
{
    RCLCPP_INFO(get_logger(),
                "🚀 Lifecycle Transition | FROM: %s (%d) → TO: ACTIVATING",
                state.label().c_str(),
                state.id());
    startDevice();
    return CallbackReturn::SUCCESS;
...
{
    RCLCPP_INFO(get_logger(),
                "🛑 Lifecycle Transition | FROM: %s (%d) → TO: DEACTIVATING",
                state.label().c_str(),
                state.id());
    stopDevice();
    return CallbackReturn::SUCCESS;
...
{
    RCLCPP_INFO(get_logger(),
                "🧹 Lifecycle Transition | FROM: %s (%d) → TO: CLEANING UP",
                state.label().c_str(),
                state.id());
    closeDevice();
    return CallbackReturn::SUCCESS;
...
{
    RCLCPP_INFO(get_logger(),
                "⚡ Lifecycle Transition | FROM: %s (%d) → TO: SHUTTING DOWN",
                state.label().c_str(),
                state.id());
    closeDevice();
    return CallbackReturn::SUCCESS;
...
RealSenseNodeFactory::RealSenseNodeFactory(const std::string &node_name,
                                           const std::string &ns,
                                           const rclcpp::NodeOptions &node_options)
    : RosNodeBase(node_name, ns, node_options), _logger(this->get_logger())
{
    init();
}
...
std::string RealSenseNodeFactory::parseUsbPort(std::string line)
{
    ...
    if (found) {
        ...
        if (base_match[2].str().size() ==
            0) // This is libuvc string. Remove counter is exists.
        {
            std::regex end_regex =
...
void RealSenseNodeFactory::getDevice(rs2::device_list list)
{
        ...
        if (0 == list.size()) {
            ...
            for (size_t count = 0; count < list.size(); count++) {
                try {
                    ...
                    ROS_WARN_STREAM("Device " << count + 1 << "/" << list.size()
                ...
                if (pid_str != "DDS") {
                    if (port_id.empty()) {
                        ...
                        msg << "Error extracting usb port from device with physical ID: "
                            << pn << std::endl
                            << "Please report on github issue at "
                        ...
                        if (_usb_port_id.empty()) {
                            ...
                            ROS_ERROR_STREAM(
                                "Please use serial number instead of usb port.");
                        }
                    } else {
                        ROS_INFO_STREAM("Device with port number " << port_id
                                                                   << " was found.");
                    }
                }
                ...
                if (!_device_type.empty()) {
                    ...
                    found_device_type =
                        std::regex_search(name, match_results, device_type_regex);
                }
            ...
            if (!found) {
                ...
                if (_device.supports(RS2_CAMERA_INFO_USB_TYPE_DESCRIPTOR)) {
                    ...
                    if (usb_type.find("2.") != std::string::npos) {
                        ROS_WARN_STREAM("Device "
                                        << _serial_no << " is connected using a "
                                        << usb_type
    ...
    if (_device && _initial_reset) {
        ...
        try {
            ...
            ROS_WARN_STREAM("An exception has been thrown: "
...
std::string api_version_to_string(int version)
{
    ...
    if (version / 10000 == 0)
        ...
        ss << (version / 10000) << "." << (version % 10000) / 100 << "."
           << (version % 100);
    return ss.str();
}
...
void RealSenseNodeFactory::init()
{
    try {
        ...
        rs2_error *e = nullptr;
        std::string running_librealsense_version(
            api_version_to_string(rs2_get_api_version(&e)));
        ...
        _usb_port_id =
            _parameters->getOrDeclareParameter<std::string>("_usb_port_id", "");
        _device_type =
            _parameters->getOrDeclareParameter<std::string>("_device_type", "");
        _wait_for_device_timeout =
            _parameters->getOrDeclareParameter<double>("wait_for_device_timeout", -1.0);
        _reconnect_timeout =
            _parameters->getOrDeclareParameter<double>("reconnect_timeout", 6.0);

        // A ROS2 hack: until a better way is found to avoid auto convertion of strings
        // containing only digits to integers:
        if (!_serial_no.empty() && _serial_no.front() == '_')
            _serial_no = _serial_no.substr(1); // remove '_' prefix

        std::string rosbag_filename(
            _parameters->getOrDeclareParameter<std::string>("rosbag_filename", ""));
        if (!rosbag_filename.empty()) {
            ...
            if (_device) {
                bool rosbag_loop(
                    _parameters->getOrDeclareParameter<bool>("rosbag_loop", false));
                startDevice();

                if (rosbag_loop) {
                    auto playback =
                        _device.as<rs2::playback>(); // Object to check the playback
                                                     // status periodically.
                    ...
                    while (rclcpp::ok()) {
                        ...
                        if (!is_playing && status == RS2_PLAYBACK_STATUS_STOPPED) {
                            RCLCPP_INFO(this->get_logger(),
                                        "Bag file playback has completed and it is going "
                                        "to be replayed.");
                            startDevice();     // Re-start bag file execution
                            is_playing = true; // Set the flag to true as playback has
                                               // been restarted
                        } else if (status != RS2_PLAYBACK_STATUS_STOPPED) {
                            // If the playback status is not stopped, it means the
                            // playback is active
                            is_playing = true;
                        } else {
            ...
            _initial_reset =
                _parameters->getOrDeclareParameter<bool>("initial_reset", false);

            _query_thread = std::thread([=]() {
                std::chrono::milliseconds timespan(
                    static_cast<int>(_reconnect_timeout * 1e3));
                    ...
                    try {
                        ...
                        if (_device) {
                            std::function<void(rs2::event_information &)>
                                change_device_callback_function =
...
// unconfigure lifecycle state ends here for lifecycled node
                            ...
                            if (_wait_for_device_timeout > 0) {
                                auto time_to_timeout(
                                    _wait_for_device_timeout -
                                    (this->get_clock()->now() - first_try_time)
                                        .seconds());
                                if (time_to_timeout < 0) {
                                    ROS_ERROR_STREAM("wait for device timeout of "
                                                     << _wait_for_device_timeout
                                                     << " secs expired");
                                    exit(1);
                                } else {
                                    double max_timespan_secs(
                                        std::chrono::duration_cast<std::chrono::seconds>(
                                            timespan)
                                            .count());
                                    actual_timespan =
                                        std::chrono::milliseconds(static_cast<int>(
                                            std::min(max_timespan_secs, time_to_timeout) *
                                            1e3));
                                }
                            }
        ...
        ROS_ERROR_STREAM("An exception has been thrown: " << __FILE__ << ":" << __LINE__
                                                          << ":" << ex.what());
        exit(1);
    } catch (...) {
...
void RealSenseNodeFactory::startDevice()
{
    ...
    try {
        ...
        case RS436_PID:
        case RS_USB2_PID:
            _realSenseNode = std::unique_ptr<BaseRealSenseNode>(new BaseRealSenseNode(
                *this,
                _device,
                _parameters,
...
void RealSenseNodeFactory::closeDevice()
{
    ...
    if (_device) {
        ...
        // To go into unconfigured lifecycle state for lifecycled node we have to also
...
void RealSenseNodeFactory::tryGetLogSeverity(rs2_log_severity &severity) const
{
    ...
    if (content) {
        ...
        std::transform(
            content_str.begin(), content_str.end(), content_str.begin(), ::toupper);
