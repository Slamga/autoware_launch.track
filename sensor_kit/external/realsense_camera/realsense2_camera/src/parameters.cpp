...
void BaseRealSenseNode::getParameters()
{
    ...
    _parameters->setParamT(param_name,
                           _tf_publish_rate,
    ...
    _hold_back_imu_for_frames =
        _parameters->setParam<bool>(param_name, HOLD_BACK_IMU_FOR_FRAMES);
    _parameters_names.push_back(param_name);

    param_name = std::string("base_frame_id");
    _base_frame_id =
        _parameters->setParam<std::string>(param_name, DEFAULT_BASE_FRAME_ID);
    // _base_frame_id = (static_cast<std::ostringstream &&>(
    //                       std::ostringstream() << _camera_name << "_" <<
    //                       _base_frame_id))
    //                      .str();
    _parameters_names.push_back(param_name);
    ...
    _parameters->setParam<bool>(
        param_name, false, [this](const rclcpp::Parameter &parameter) {
...
void BaseRealSenseNode::setDynamicParams()
{
    ...
    for (auto vec_iter : enum_vec) {
        enum_str_values << std::setw(longest_desc + 6) << std::left << vec_iter.first
