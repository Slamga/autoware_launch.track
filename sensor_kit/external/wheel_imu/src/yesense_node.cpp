...
#include "wheel_imu/msg/attitude_all_data.hpp"
#include "wheel_imu/msg/attitude_min_ahrs.hpp"
#include "wheel_imu/msg/attitude_min_vru.hpp"
#include "wheel_imu/msg/euler_only.hpp"
#include "wheel_imu/msg/imu_data.hpp"
#include "wheel_imu/msg/imu_data_ten_axis.hpp"
#include "wheel_imu/msg/nav_all.hpp"
#include "wheel_imu/msg/nav_min.hpp"
#include "wheel_imu/msg/nav_min_utc.hpp"
#include "wheel_imu/msg/pos_only.hpp"
#include "wheel_imu/msg/robot_lord.hpp"
...
int g_baudrate_table[BAUDRATE_CNT] = {
    9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600}; // 目前仅支持这些波特率

int g_baudrate_table_linux[BAUDRATE_CNT] = {B9600,
                                            B19200,
                                            B38400,
                                            B57600,
                                            B115200,
...
class YESENSE_Publisher : public rclcpp::Node {
  public:
    YESENSE_Publisher() : Node("yesense_publisher")
    {
        ...
        pub_imu_ros =
            this->create_publisher<sensor_msgs::msg::Imu>(imu_topic_ros.c_str(), 10);
        pub_pose_geometry =
        ...
        // ==========================================
        pub_imu = this->create_publisher<wheel_imu::msg::ImuData>(imu_topic.c_str(), 10);
        pub_sensor_10axis =
            this->create_publisher<wheel_imu::msg::ImuDataTenAxis>("sensor_10axis", 10);
        pub_euler = this->create_publisher<wheel_imu::msg::EulerOnly>("euler_only", 10);
        pub_robt_lord =
            this->create_publisher<wheel_imu::msg::RobotLord>("robot_lord", 10);
        pub_att_min_vru =
            this->create_publisher<wheel_imu::msg::AttitudeMinVru>("att_min_vru", 10);
        pub_att_min_ahrs =
            this->create_publisher<wheel_imu::msg::AttitudeMinAhrs>("att_min_ahrs", 10);
        pub_att_all =
            this->create_publisher<wheel_imu::msg::AttitudeAllData>("att_all", 10);
        pub_pos = this->create_publisher<wheel_imu::msg::PosOnly>("pos_only", 10);
        pub_nav_min = this->create_publisher<wheel_imu::msg::NavMin>("nav_min", 10);
        pub_nav_min_utc =
            this->create_publisher<wheel_imu::msg::NavMinUtc>("nav_min_utc", 10);
        pub_nav_all = this->create_publisher<wheel_imu::msg::NavAll>("nav_all", 10);
        ...
        *((unsigned int *) &yis_out) = 0u;
        ...
        } else if (0 == memcmp(driver_type_str.c_str(),
                               "linux_serial",
                               strlen("linux_serial"))) {
  ...
  private:
    void timer_callback()
    {
        ...
        int ret =
            decoder.data_proc(r_buffer, (unsigned int) bytes_read_r_buffer, &yis_out);
        ...
        if (analysis_ok == ret) {
            if (yis_out.content.valid_flg) {
                ...
                // RCLCPP_INFO(this->get_logger(), "msg rate: %d, tid %d, acc: %f, %f, %f
                // gyro: %f, %f, %f, euler: %f, %f, %f!",
                // user_info.msg_rate, 			yis_out.tid,
                // yis_out.acc.x, yis_out.acc.y, yis_out.acc.z, yis_out.gyro.x,
                // yis_out.gyro.y, yis_out.gyro.z, yis_out.euler.pitch,
                // yis_out.euler.roll, yis_out.euler.yaw
                //		);
                //
            }
        }
    ...
    rclcpp::Publisher<wheel_imu::msg::ImuData>::SharedPtr
        pub_imu; // only inertia data: tid/acc/gyro/sensor_temp/sample_timestamp
    rclcpp::Publisher<wheel_imu::msg::ImuDataTenAxis>::SharedPtr
        pub_sensor_10axis; // only inertia data:
                           // tid/acc/gyro/sensor_temp/sample_timestamp/mag/pressure(reserved)
    rclcpp::Publisher<wheel_imu::msg::EulerOnly>::SharedPtr
        pub_euler; // only euler: tid/pitch/roll/yaw
    rclcpp::Publisher<wheel_imu::msg::RobotLord>::SharedPtr
        pub_robt_lord; // tid/acc/gyro/quatenion
    rclcpp::Publisher<wheel_imu::msg::AttitudeMinVru>::SharedPtr
        pub_att_min_vru; // minimum recommended data for attitude vru product: imu/euler
    rclcpp::Publisher<wheel_imu::msg::AttitudeMinAhrs>::SharedPtr
        pub_att_min_ahrs; // minimum recommended data for attitude ahrs product:
                          // imu/mag/euler
    rclcpp::Publisher<wheel_imu::msg::AttitudeAllData>::SharedPtr
        pub_att_all; // attitude all data: imu/mag/euler/quat
    rclcpp::Publisher<wheel_imu::msg::PosOnly>::SharedPtr
        pub_pos; // only position: tid/latitude/longitude/altitude/nav_state
    rclcpp::Publisher<wheel_imu::msg::NavMin>::SharedPtr
        pub_nav_min; // recommended data for navigation product:
                     // tid/postion/euler/nav_state
    rclcpp::Publisher<wheel_imu::msg::NavMinUtc>::SharedPtr
        pub_nav_min_utc; // recommended data for navigation product with utc:
                         // tid/postion/euler/utc/nav_state
    rclcpp::Publisher<wheel_imu::msg::NavAll>::SharedPtr
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
    ...
    wheel_imu::msg::ImuData imu_data;
    wheel_imu::msg::ImuDataTenAxis sensor_data_10axis;
    wheel_imu::msg::EulerOnly euler_data;
    wheel_imu::msg::RobotLord robot_data;
    wheel_imu::msg::AttitudeMinVru att_min_vru_data;
    wheel_imu::msg::AttitudeMinAhrs att_min_ahrs_data;
    wheel_imu::msg::AttitudeAllData att_all_data;
    wheel_imu::msg::PosOnly pos_data;
    wheel_imu::msg::NavMin nav_min_data;
    wheel_imu::msg::NavMinUtc nav_min_utc_data;
    wheel_imu::msg::NavAll nav_all_data;
